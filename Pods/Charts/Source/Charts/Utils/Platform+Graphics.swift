...
func NSUIImagePNGRepresentation(_ customImage: NSUIImage) -> Data?
{
    return customImage.pngData()
}

func NSUIImageJPEGRepresentation(_ customImage: NSUIImage, _ quality: CGFloat = 0.8) -> Data?
{
    return customImage.jpegData(compressionQuality: quality)
}
...
func NSUIImagePNGRepresentation(_ customImage: NSUIImage) -> Data?
{
    ...
    let rep = NSBitmapImageRep(focusedViewRect: NSMakeRect(0, 0, customImage.size.width, customImage.size.height))
    customImage.unlockFocus()
    return rep?.representation(using: .png, properties: [:])
}

func NSUIImageJPEGRepresentation(_ customImage: NSUIImage, _ quality: CGFloat = 0.9) -> Data?
{
    customImage.lockFocus()
    let rep = NSBitmapImageRep(focusedViewRect: NSMakeRect(0, 0, customImage.size.width, customImage.size.height))
    customImage.unlockFocus()
...
func NSUIGraphicsGetImageFromCurrentImageContext() -> NSUIImage?
{
    if !imageContextStack.isEmpty
    {
        ...
        if let theCGImage = ctx.makeImage()
        {
            ...
            let customImage = NSImage(cgImage: theCGImage, size: size)
            return customImage
        }
    }
    return nil
